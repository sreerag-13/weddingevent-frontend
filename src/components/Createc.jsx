...
import Navcat from './Navcat'; // Ensure this is the correct path for your Navcat component
...
const Createc = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Try to upload the images to the catering endpoint
    try {
      const response = await axios.post("http://localhost:8082/create-catering-post", formData, {
        headers: {
          "Content-Type": "multipart/form-data"
        }
      });
      setMessage("Catering post created successfully!"); // Success message
      console.log("Server response:", response.data); // Log success response for debugging
    } catch (error) {
      console.error("Error uploading images:", error.response?.data || error.message); // Log error response for debugging
  ...
  return (
    <div>
      <Navcat /> {/* Include the Navcat navigation component */}
      <div className="container">
        <h2>Create a New Catering Post</h2>
